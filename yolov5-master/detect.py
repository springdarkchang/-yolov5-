...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    data_num = dataset.get_nf()
    results = [0] * data_num * 2
    for path, im, im0s, vid_cap, s in dataset:
        cur_num = int(s.split(' ')[1].split('/')[0]) - 1
        t1 = time_sync()
...
def main(opt):
    ...
    for i in range(len(results) // 2):
        if results[i + len(results) // 2] > 0:
            point = 60
        else:
            point = 0
        if results[i] > point:
            print(f"第{i + 1}个文件中出现了测温枪")
        else:
            print(f"第{i + 1}个文件中未出现测温枪")
